...
while true; do
    ...
    if [ $battery_capacity -le 20 ] && [ $battery_capacity -gt 10 ] && [ "$last_notification" != "low" ]; then
    ...
    elif [ $battery_capacity -le 10 ] && [ $battery_capacity -gt 5 ] && [ "$last_notification" != "very low" ]; then
        dunstify -u critical -h string:x-dunst-stack-tag:battery \
            "Very low battery!" "The battery capacity is now at $battery_capacity%." && \
            last_received_notification="very low"

    elif [ $battery_capacity -le 5 ] && [ "$last_notification" != "critical" ]; then
        dunstify -u critical -h string:x-dunst-stack-tag:battery \
